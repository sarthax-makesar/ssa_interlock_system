...
module lost_drive_watchdog (
    ...
    // =========================================================================
    localparam [15:0] THRES_DRIVE_MIN = 16'h0B33; // Rule 5: Drive floor limit
    ...
    // =========================================================================
    // Rule 1 & 2 & 4: Watchdog Kick (Reset) Conditioning Logic
    // =========================================================================
    ...
    gc_fsm_watchdog #(
        ...
        .wdt_rst_i (wdt_rst_conditioned),         // Hook up our conditioned kick logic[cite: 11]
        .fsm_rst_o (wdt_fsm_rst_pulse)            // 1-cycle raw strobe output[cite: 11]
    );

    // =========================================================================
    // Rule 3 & 4: Output Catching & Re-arming Latch Matrix
    // =========================================================================
    always @(posedge clk_fast or negedge rst_n) begin
        if (!rst_n) begin
            lost_drive_trip_latch <= 1'b0;
        end 
        // Rule 4: Master interlock reset clears the output latch to re-arm the system
        else if (interlock_reset_i) begin
            lost_drive_trip_latch <= 1'b0;
        end 
        // Rule 3: Catch the 1-clock-cycle pulse from the VHDL timer and latch it permanently[cite: 11]
